...
```,
  caption: [Трејт Detector (analysis_service/src/detectors/mod.rs)]
) <lst-6>
...
`PerformanceDetector` пријављује три врсте проблема: дубоко угњеждене петље високе сложености (цикломатска сложеност > 15 и дубина угњеждавања > 3), потенцијалне Н+1 упите над базом података унутар тела петље, и конкатенацију стрингова оператором `+=` унутар петље (O(n²) понашање).
...
Suggestion Generator Service представља последњу карику pipeline-a: за сваки рангирани проблем генерише конкретан, језички прилагођен предлог исправке, уписује га у MongoDB колекцију `suggestions`, и — с обзиром да је последњи сервис у ланцу — ажурира напредак посла у PostgreSQL табели `analysis_jobs`, укључујући и превођење посла у статус `completed` када су сви фајлови обрађени.
